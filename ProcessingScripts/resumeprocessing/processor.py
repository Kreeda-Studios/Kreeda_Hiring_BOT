...
# ──────────────────────────────────────────────
# PROCESS SINGLE RESUME (FOR BULLMQ WORKER)
# ──────────────────────────────────────────────

async def process_single_resume_file(pdf_path: Path) -> Dict[str, Any]:
    """
    Process a single resume file and return extracted data
    
    This function is called by the BullMQ worker with a temporary file path.
    It extracts content from the PDF and processes it with AI.
    
    Args:
        pdf_path: Path to the PDF file (can be a temporary file)
    
    Returns:
        Dictionary containing extracted resume data or error
    
    Example:
        >>> from pathlib import Path
        >>> result = await process_single_resume_file(Path('/tmp/resume_abc.pdf'))
    ...
    try:
        ...
        if not raw_text:
            ...
            return {"error": "Invalid resume text - no content could be extracted"}
        
        # Send to AI for structured extraction
        logger.info(f"🤖 Sending to AI for extraction...")
