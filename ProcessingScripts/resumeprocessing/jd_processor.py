"""
JD (Job Description) Processing Engine
========================================
Extracts structured information from job description PDF files using OpenAI.

Flow (worker mode):
1. Receives path to downloaded JD PDF  (called by jd_handler.py)
2. Extracts raw text from the PDF
3. Sends extracted text to OpenAI for structured extraction
4. Returns structured dict to jd_handler.py  → saved to MongoDB

Flow (standalone / batch mode):
    python -m resumeprocessing.jd_processor
    Reads PDFs from input_jds/, writes JSON to output_json/

Usage:
    from resumeprocessing.jd_processor import process_single_jd_file
    result = await process_single_jd_file(pdf_path)
"""
...
# ──────────────────────────────────────────────
# WORKER ENTRY POINT  (called by jd_handler.py)
# ──────────────────────────────────────────────

async def process_single_jd_file(pdf_path: Path) -> Dict[str, Any]:
    """
    Process a single JD PDF and return structured extracted data.

    Called by jd_handler.py for each BullMQ job.

    Args:
        pdf_path: Path to the downloaded JD PDF (temp file)

    Returns:
        Dictionary matching the JDExtraction schema.
        On error: {"error": "<message>"}
    """
    logger.info(f"📄 Processing JD: {pdf_path.name}")

    raw_text = extract_pdf_content(str(pdf_path))

    if not raw_text:
        logger.warning(f"⚠️  No text extracted from {pdf_path.name}")
        return {"error": "Could not extract text from JD PDF"}
    ...
    logger.info(f"   ✅ JD extraction complete")
    return result
